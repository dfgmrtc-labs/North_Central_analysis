...
if(params.in_data_type == "pe_illumina_reads" | params.in_data_type == "pe_only"){
    params.pe_reads = params.seq_path + "sample_*_{1,2}.fastq.gz"
    ...
    params.se_reads = params.seq_path + "sample_*.fastq.gz"
    ...
    params.se_reads = params.seq_path_se + "sample_*.fastq.gz"
    params.pe_reads = params.seq_path_pe + "sample_*_{1,2}.fastq.gz"
    params.pe_reads_channel = Channel.fromFilePairs(params.pe_reads)
    params.se_reads_channel = Channel.fromPath(params.se_reads)
}
...
include {trim_fastq; emit_sam; coordsort_sam; bcf_call; gatk_call; gatk_call_gvcf; prepare_manifest; adjudicate_var; minos_joint_genotyping; gatk_joint_genotyping; isec; emit_sam_pe_min} from "./bin/module_script.nf"
...
workflow call_genotype_var {
    take:
        ...
        tandem_qual_wf
        indel_bias_wf
        cohort_joint_genotype
        min_frs_wf
        min_gcp_wf

    main:
        
        // Option Joint genotype cohorts
        if (params.cohort_joint_genotype == "ncgvcf"){
            ...
            trimmed_reads_2 = "no_se"
            emit_sam(emit_sam_script_wf, trim_fastq.out.trimmed_reads, trimmed_reads_2, fastafile, input_read_type)
            //emit_sam(emit_sam_script_wf, pe_reads_wf, trimmed_reads_2, fastafile, input_read_type)
        ...
        // Call variants
            ...
            if (params.in_data_type != "minion_ont_reads"){
                ...
                if (params.sample_count == "gvcf"){
                    ...
                    adjudicate_var(bcf_call.out, gatk_call.out, trim_fastq.out.trimmed_reads, var_adj_script_wf, input_read_type, fastafile, min_frs_wf, min_gcp_wf)
                    //adjudicate_var(bcf_call.out, gatk_call.out, pe_reads_wf, var_adj_script_wf, input_read_type, fastafile, min_frs_wf, min_gcp_wf)
                }
            ...
            trim_fastq(trim_sample_script_wf, pe_reads_wf, adapter_seq, input_read_type, se_reads_wf)
            //emit_sam_pe_min(emit_sam_script_wf, trim_fastq.out.trimmed_reads, trim_fastq.out.trimmed_reads_2, fastafile, input_read_type)
            //println(trim_fastq.out.trimmed_reads_2)
            if (params.in_data_type == "pe_min"){
                emit_sam(emit_sam_script_wf, trim_fastq.out.trimmed_reads, trim_fastq.out.trimmed_reads_2, fastafile, input_read_type)
            } else if (params.in_data_type == "pe_only"){
                trimmed_reads_2 = "no_se"
                emit_sam(emit_sam_script_wf, trim_fastq.out.trimmed_reads, trimmed_reads_2, fastafile, input_read_type)
            }
            coordsort_sam(coordsort_sam_script_wf, emit_sam.out)
            gatk_call_gvcf(gatk_call_script_wf, coordsort_sam.out, fastafile, min_BQ_wf, ploidy_wf, cohort_joint_genotype)
            flattened_gvcfs = gatk_call_gvcf.out | flatten | collect
            gatk_joint_genotyping(gatk_joint_geno_script_wf, flattened_gvcfs, fastafile)
...
workflow {
    call_genotype_var(Channel.value(params.trim_sample_script), Channel.value(params.emit_sam_script), Channel.value(params.coordsort_sam_script), Channel.value(params.bcf_call_script), 
    Channel.value(params.gatk_call_script), Channel.value(params.var_adj_script), Channel.value(params.gatk_joint_geno_script), params.se_reads_channel, params.pe_reads_channel, Channel.value(params.ref_seq_path_int), Channel.value(params.adp_path_int),
    Channel.value(params.in_data_type), Channel.value(params.minos_config), Channel.value(params.sample_size_profile), Channel.value(params.min_BQ), Channel.value(params.min_MQ),
    Channel.value(params.ploidy), Channel.value(params.tandem_qual) , Channel.value(params.indel_bias), Channel.value(params.cohort_joint_genotype), Channel.value(params.frs), Channel.value(params.gcp))    
}


///nextflow main.nf --in_data_type minion_ont_reads --seq_path /home/dfgmrtc/TESTING/Raw_fastq_files/New_ONT/complete/ --adp_path nextera_pe_path --ref_seq_path h37rv.fasta --out_dir ~/Workflows/wf-vdg/With_GQ --sample_count  gvcf --frs 90 –gcp 0.5